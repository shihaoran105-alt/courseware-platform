...
rem ---------------------------------------------------------------
rem 桌面快捷方式。
rem 注意：不能用 echo 重定向去写这个 .cmd —— 那样写出来的是不带 BOM 的
rem UTF-8，下次双击时 cmd 会用系统代码页（中文系统 GBK）去读，中文全乱。
rem 所以交给 PowerShell 明确按 GBK（代码页 936）写，不带 BOM、也不切代码页。
rem 带 BOM 或 chcp 65001 同样会让 cmd 在中文系统上解析出错。
rem ---------------------------------------------------------------
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  "$lines = @('@echo off','cd /d \"%INSTALL_DIR%\"','call \"一键安装并启动-Windows.cmd\"');" ^
