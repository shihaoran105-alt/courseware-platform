...
# GitHub 源码包和部分第三方解压工具会丢失可执行权限。
# 只要本文件是通过 bash 启动的，就顺手修复后续脚本。
chmod +x "$0" ./start.sh ./检查更新.command 2>/dev/null || true
...
if [ -d "$desktop_dir" ]; then
  {
    ...
    printf 'cd %q || exit 1\n' "$project_dir"
    echo 'exec bash ./一键安装并启动.command'
  } > "$desktop_launcher"
...
# 通过 bash 调用，即使 start.sh 的可执行位丢失也能启动。
exec bash ./start.sh
